  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!profile) {
          setErrorMessage('Could not load your admin profile.')
          setLoading(false)
          return
        }
        ...
        setErrorMessage('Could not load admin setup.')
        setLoading(false)
      }
    }
    ...
    return (
      <div style={{ padding: 24 }}>
        <div className="card" style={{ padding: 18 }}>
          <div style={{ fontSize: 14, color: 'var(--color-ink-soft)' }}>Preparing admin setup…</div>
        </div>
      </div>
    )
  }
    ...
    <div style={{ padding: '20px 24px 28px' }}>
      <div className="card" style={{ padding: '18px 18px 16px', marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--color-ink-soft)', lineHeight: 1.55 }}>
          Add your school once, then you can invite teachers and generate parent links.
        </div>
      </div>
